...
def market_open(context):
    ...
    # 获取股票五天的收盘价
    close_data: numpy.ndarray = get_bars(
        security, 
        count=5, 
        unit='1d', 
        fields=['close']
    ...
    onehand = close_data['close'][0] * 100
    # 如果上一时间点价格高出五天平均价1%, 则全仓买入
    if (current_price > 1.01 * MA5) and (cash > onehand): # 一手
        # 记录这次买入
        log.info("价格高于均价 1%%, 买入 %s" % (security))
        # 用所有 cash 买入股票
        adjusted_cash = (cash // onehand) * (onehand)
    ...
    # 如果上一时间点价格低于五天平均价, 则空仓卖出
        ...
        if current_price < MA5 and context.portfolio.positions[security].closeable_amount > 0:
            ...
            order_target(security, amount=0)

## 收盘后运行函数(需要提前注册)
def after_market_close(context):
    log.info(str('函数运行时间(after_market_close):'+str(context.current_dt.time())))
    #得到当天所有成交记录
    trades = get_trades()# dict
